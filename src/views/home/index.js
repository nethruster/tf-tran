...
export default connect('routes', actions)(class Home extends Component {
  constructor(props) {
    super(props)
  ...
  renderRouteStops(route, stops) {
    ...
    return Object.keys(stops).map(stop => {
      return (
        <Link to={`/route/${route}/stop/${stop}`} class={style.listingItemLink}>
  ...
  renderLines() {
    ...
    return this.props.routes.map(route => {
      return ( 
        <div class={`flex flex-main-center ${style.lineListingContainer}`} data-line={route.name}>
          <ul>
            {this.renderRouteStops(route.name, route.stops)}
          </ul>
        </div>
      )})
  }
  ...
  render({routes}) {
    return (
      <div class={style.homeWrapper}>
       { routes === null ? <p>Cargando datos...</p> : this.renderLines() }
      </div>
    )
  }
})
